...
ec2-attach-volume ${UNIKERNELVOLUMEID} --aws-access-key ${AWS_ACCESS_KEY} --aws-secret-key ${AWS_SECRET_KEY} --region ${THISREGION} --instance ${THISINSTANCEID} --device /dev/xvdf
...
echo ec2-run-instances --aws-access-key ${AWS_ACCESS_KEY} --aws-secret-key ${AWS_SECRET_KEY} --region ${THISREGION} ${AMIID}
echo ""

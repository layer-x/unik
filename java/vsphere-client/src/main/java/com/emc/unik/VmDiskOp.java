...
public class VmDiskOp
{
            ...
            VirtualMachineConfigInfo vmConfig, String name)
    {
        ...
        {
            if(devices.get(i).getDeviceInfo().getLabel().equals(name))
            {
            ...
            VirtualMachine vm, int diskSize, String diskMode, String diskName) throws Exception
    {
        ...
        List<VirtualDevice> vds = vmConfig.getHardware().getDevice();
        ...
        for(int k=0;k<vds.size();k++)
        {
            ...
            {
                key = vds.get(k).getKey();
            }
        }

        unitNumber = vds.size() + 1; //***********************seems NOT correct!!!

...
func (root *Chiroot) getMainClass() (string, error) {
	if root.Chiproject.Chibuild != nil {
		for _, plugin := range root.Chiproject.Chibuild.Chiplugins.Chiplugin {
			if plugin.Chiconfiguration != nil &&
			plugin.Chiconfiguration != nil &&
			plugin.Chiconfiguration.ChidescriptorRefs != nil {
				for _, pluginDescriptor := range plugin.Chiconfiguration.ChidescriptorRefs.ChidescriptorRef {
					if pluginDescriptor.Text == "jar-with-dependencies" {
